...
export async function updateDirectory(req, res) {
  try {
    ...
    dateFields.forEach((field) => {
      if (updates[field]) {
        ...
        if (!isNaN(parsedDate)) {
          ...
          delete updates[field]; // Remove invalid date
        }
      }
    });
    ...
    if (updates.stepIncrement !== undefined) {
      updates.stepIncrement = Number(updates.stepIncrement) || 0;
    }

    // Update the document in DB
    const updatedDirectory = await Directory.findByIdAndUpdate(id, updates, {
      new: true,
      runValidators: true,
    });
